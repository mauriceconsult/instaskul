...
export const AdminCoursesList = ({ items, adminId }: AdminCoursesListProps) => {
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    try {
      await axios.put(`/api/admins/${adminId}/courses/reorder`, { list: reorderData });
      toast.success("Courses reordered");
      router.refresh();
    } catch {
      toast.error("Failed to reorder");
      setCourses(items); // revert on error
    }
  };

  const onEdit = (id: string) => {
    router.push(`/dashboard/admins/${adminId}/courses/${id}`);
  };
