// app/api/coursenoticeboards/[noticeboardId]/comments/route.ts
...
export async function GET(
  ...
  try {
    ...
    const comments = await prisma.courseNoticeboardComment.findMany({
      where: {
        courseNoticeboardId: noticeboardId,
        courseId: null, // Only top-level comments
      },
      include: {
        replies: {
          orderBy: {
    ...
    console.error("[COURSE_NOTICEBOARD_COMMENTS_GET]", error);
...
export async function POST(
  ...
  try {
    ...
    const comment = await prisma.courseNoticeboardComment.create({
      data: {
        courseNoticeboardId: noticeboardId,
        userId,
    ...
    console.error("[COURSE_NOTICEBOARD_COMMENTS_POST]", error);
    return new NextResponse("Internal Error", { status: 500 });
  }
}
